...
// Custom hooks
import { useDebitNoteForm } from "@/admin/hooks/useDebitNoteForm";

// Components
import DebitNoteGeneralInfo from "@/admin/components/shared/notes/DebitNoteGeneralInfo";
import NoteAccountingTabs from "@/admin/components/shared/notes/NoteAccountingTabs";
import { Button } from "@/admin/components/ui/button";
import DebitNotePrint from "@/admin/components/shared/print/DebitNotePrint";
...
export default function DebitNoteForm() {
    ...
    const handleSubmit = useCallback(
        (e) => {
            const submitRoute = isEdit
                ? route("admin.note.debit.update", debit_note.id)
                : route("admin.note.debit.store");
            const submitMethod = isEdit ? "put" : "post";

            baseHandleSubmit(e, submitRoute, submitMethod);
        },
        [isEdit, debit_note, baseHandleSubmit],
    );

    // Xử lý in
    const handlePrint = useReactToPrint({
        contentRef: printRef,
        documentTitle: `GBN-${formData.code || "Moi"}`,
        pageStyle: `
            @page {
                size: A4;
                margin: 10mm;
        ...
        onPrintError: () =>
            emit("toast:error", "Có lỗi khi in giấy báo nợ! Vui lòng thử lại."),
    });
    ...
    const statusBadge = formData.status
        ...
        : null;
    const canPrint =
        isEdit && debit_note?.status === "confirmed" && !!formData.code;

    return (
        <AdminLayout
            breadcrumb={[
                { label: "Dashboard", link: route("admin.dashboard.index") },
                { label: "Giấy báo nợ", link: route("admin.note.debit.index") },
                {
                    label: isEdit
                        ? `Chỉnh sửa ${formData.code || "giấy báo nợ"}`
                        : "Thêm giấy báo nợ",
                },
            ]}
        >
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold">
                                    {isEdit
                                        ? `Chỉnh sửa giấy báo nợ: ${formData.code}`
                                        : "Thêm giấy báo nợ mới"}
                                </h1>
