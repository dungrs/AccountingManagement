...
import DebitNoteTable from "@/admin/components/pages/debit-note/DebitNoteTable";

export default function DebitNoteHome() {
    useFlashToast();
    ...
    const fetchData = useCallback(
        async (page = 1) => {
            ...
            try {
                ...
                if (partyTypeFilter !== "all") {
                    params.party_type = partyTypeFilter;
                }

                const res = await axios.post(
                    route("admin.note.debit.filter"),
                    params,
                );
                ...
                const mappedData = response.data.map((item) => ({
                    ...
                    party_type: item.customer_id ? "customer" : "supplier",
                ...
                console.log("Dữ liệu giấy báo nợ:", mappedData);
        ...
        [pageSize, debouncedKeyword, statusFilter, typeFilter, partyTypeFilter],
    );
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const res = await axios.post(
                route("admin.note.debit.delete", deletingRow.id),
            );
    ...
    const customerCount = data.filter(
        (item) => item.party_type === "customer",
    ).length;
    const supplierCount = data.filter(
        (item) => item.party_type === "supplier",
    ).length;
    ...
    return (
        <AdminLayout
            breadcrumb={[
                ...
                {
                    label: "Quản Lý Giấy Báo Nợ",
                },
            ]}
        >
            <Head title="Quản Lý Giấy Báo Nợ" />
                    ...
                    <CardContent className="p-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge className="bg-blue-200 text-blue-800 border-blue-300 text-[10px] h-5">
                                {((customerCount / (data.length || 1)) * 100).toFixed(1)}%
                            </Badge>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="p-3">
                        <div className="flex items-center justify-between">
                            ...
                            <Badge className="bg-purple-200 text-purple-800 border-purple-300 text-[10px] h-5">
                                {((supplierCount / (data.length || 1)) * 100).toFixed(1)}%
                            </Badge>
                        </div>
                    </CardContent>
                </Card>
            ...
            <Card className="rounded-md shadow-lg border-slate-200 overflow-hidden">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <CardDescription className="text-white/80">
                                Quản lý danh sách giấy báo nợ, khách hàng/nhà cung cấp, số tiền và trạng thái.
                            </CardDescription>
                        </div>
                ...
                <CardContent className="p-6 space-y-4">
                    <DataTableFilter
                        ...
                        placeholder="Tìm kiếm theo mã, đối tượng, lý do..."
                        className="bg-white"
                    >
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                                <SelectTrigger className="w-full sm:w-[150px] rounded-md border-slate-200 focus:ring-blue-500">
                            ...
                            >
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="discount_after">
                            ...
                            <Select
                                ...
                                onValueChange={setPartyTypeFilter}
                            >
                                <SelectTrigger className="w-full sm:w-[150px] rounded-md border-slate-200 focus:ring-green-500">
            ...
            <ConfirmDeleteDialog
                ...
                title="Xóa giấy báo nợ"
                description={`Bạn có chắc chắn muốn xóa giấy báo nợ "${deletingRow?.code}" không?`}
                onCancel={() => {
                    setOpenDeleteDialog(false);
                    setDeletingRow(null);
                }}
